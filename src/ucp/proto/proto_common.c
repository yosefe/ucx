...
ucs_status_t
ucp_proto_common_lane_perf_attr(ucp_worker_iface_t *wiface,
                                uct_ep_operation_t op, uint64_t uct_field_mask,
                                ...
                                ucp_proto_perf_node_t **perf_node_p)
{
    ...
    tl_rsc    = &wiface->worker->context->tl_rscs[rsc_index].tl_rsc;
    perf_node = ucp_proto_perf_node_new_data(uct_ep_operation_names[op],
                                             UCT_TL_RESOURCE_DESC_FMT,
                                             UCT_TL_RESOURCE_DESC_ARG(tl_rsc));

    if (uct_field_mask & UCT_PERF_ATTR_FIELD_BANDWIDTH) {
        if (perf_attr->bandwidth.dedicated > UCP_PROTO_PERF_EPSILON) {
            ucp_proto_perf_node_add_bandwidth(perf_node, "bw/proc",
                                              perf_attr->bandwidth.dedicated);
        }
        if (perf_attr->bandwidth.shared > UCP_PROTO_PERF_EPSILON) {
            ucp_proto_perf_node_add_bandwidth(perf_node, "bw/node",
                                              perf_attr->bandwidth.shared);
        }
    }

    if (uct_field_mask & UCT_PERF_ATTR_FIELD_LATENCY) {
        ucp_proto_perf_node_add_scalar(perf_node, "lat", perf_attr->latency.c);
        ucp_proto_perf_node_add_scalar(perf_node, "lat/ep",
                                       perf_attr->latency.m);
    }

    if (uct_field_mask & UCT_PERF_ATTR_FIELD_SEND_PRE_OVERHEAD) {
        ucp_proto_perf_node_add_scalar(perf_node, "send-pre",
                                       perf_attr->send_pre_overhead);
    }

    if (uct_field_mask & UCT_PERF_ATTR_FIELD_SEND_POST_OVERHEAD) {
        ...
        const char *perf_fmt, ...)
{
    ...
    sys_perf_node = ucp_proto_perf_node_new(UCP_PROTO_PERF_NODE_TYPE_DATA,
                                            perf_name, perf_fmt, ap);
    va_end(ap);
                               ...
                               ucp_proto_perf_node_t **perf_node_p)
{
    ...
    status = ucp_proto_common_lane_perf_attr(
            wiface, params->send_op,
        ...
        ucp_lane_index_t *lanes)
{
    ...
    ucs_for_each_bit(lane, lane_map) {
        ...
        dst_md_index = ep_config_key->lanes[lane].dst_md_index;
        ...
        if (flags & UCP_PROTO_COMMON_INIT_FLAG_REMOTE_ACCESS) {
            ...
            if (((md_attr->cap.flags & UCT_MD_FLAG_NEED_RKEY) ||
                 ...
                 * available */
                ucs_trace("%s: dst_md_index %u not part of rkey md_map "
                          "0x%" PRIx64,
                          lane_desc, dst_md_index, rkey_config_key->md_map);
                continue;
            }
                                  ...
                                  ucp_proto_perf_node_t **perf_node_p)
{
    ...
    status = ucp_proto_common_lane_perf_attr(
            ucp_worker_iface(worker, ep_config->key.lanes[lane].rsc_index),
            memtype_op,
            ...
            UCT_PERF_ATTR_FIELD_SEND_POST_OVERHEAD |
            UCT_PERF_ATTR_FIELD_RECV_OVERHEAD |
            UCT_PERF_ATTR_FIELD_BANDWIDTH | UCT_PERF_ATTR_FIELD_LATENCY,
            &perf_attr, &tl_perf_node);
    if (status != UCS_OK) {
        return status;
    }
    ...
    ucp_proto_perf_node_add_child(perf_node, tl_perf_node);
