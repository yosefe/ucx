...
typedef struct {
    ...
    ucs_linear_func_t value; /* Entry value */
...
// TODO when testing, verify the perf_node is NOT null
                                  ...
                                  const ucs_linear_func_t *value)
{
    ...
    data = ucs_array_append(ucp_proto_perf_node_data, &perf_node->data,
...
{
    ucs_linear_func_t linear_func = ucs_linear_func_make(value, 0);

    if (perf_node == NULL) {
        return;
    }

    ucp_proto_perf_node_add_data(perf_node, name, &linear_func);
}

void ucp_proto_perf_node_add_bandwidth(ucp_proto_perf_node_t *perf_node,
                                       const char *name, double value)
{
    ucs_linear_func_t linear_func = ucs_linear_func_make(0, 1.0 / value);
                                     ...
                                     ucs_string_buffer_t *strb)
{
    ...
    if (ucs_linear_func_is_zero(data->value, UCP_PROTO_PERF_EPSILON)) {
        /* Add space to prevent empty font tag */
        ucs_string_buffer_appendf(strb, " ");
        return;
    }
                                 ...
                                 int highlight, ucs_string_buffer_t *strb)
{
    ...
    id = kh_size(nodes_hash); /* We never remove form the hash */
    ...
    /* Data entries */
    if (!ucs_array_is_empty(&perf_node->data) ||
        !ucs_string_is_empty(perf_node->desc)) {
        ucs_string_buffer_appendf(strb,
                                  "<font face=\"calibri\" point-size=\"13\">");
        if (!ucs_string_is_empty(perf_node->desc)) {
            ucs_string_buffer_appendf(strb, "%s", perf_node->desc);
            if (!ucs_array_is_empty(&perf_node->data)) {
                ucp_proto_perf_graph_str_append_line_break(strb);
            }
        }
...
static char ucp_proto_debug_fix_filenamefix_filename(char ch)
{
    if ((ch == ']') || (ch == ')') || (ch == '}')) {
        return '\0';
                            ...
                            const ucp_proto_select_elem_t *select_elem)
{
    ...
    ucp_proto_select_param_dump(worker, ep_cfg_index, rkey_cfg_index,
                                ...
                                &sel_param_strb);
    ucs_string_buffer_translate(&ep_cfg_strb, ucp_proto_debug_fix_filenamefix_filename);
    ucs_string_buffer_translate(&sel_param_strb, ucp_proto_debug_fix_filenamefix_filename);
