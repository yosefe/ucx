...
ucs_status_t ucp_proto_multi_init(const ucp_proto_multi_init_params_t *params)
{
    ...
    mpriv->reg_md_map = params->prereg_md_map;
    ...
    ucs_for_each_bit(lane, lane_map) {
        ...
        ucs_assert(lane_perf->bandwidth > 1.0);
        ucs_assert(lane_perf->latency >= 0);
        ucs_assert(lane_perf->overhead >= 0);
        ucs_assert(lane_perf->max_frag > 0);
        ucs_assert(lane_perf->min_frag >= 0);
        ucs_assert(lane_perf->sys_latency >= 0);

        lpriv->weight   = ucs_proto_multi_calc_weight(lane_perf->bandwidth,
                                                      perf.bandwidth);
        lpriv->max_frag = ucs_double_to_sizet(lane_perf->bandwidth /
                                                      max_frag_ratio,
                                              params->max_frag);
        ucs_assert(lpriv->max_frag <= lane_perf->max_frag);
    ...
    *params->super.super.priv_size = sizeof(ucp_proto_multi_priv_t) +
    ...
    ucp_proto_common_init_caps(&params->super, &perf, reg_md_map);
    return UCS_OK;
}
