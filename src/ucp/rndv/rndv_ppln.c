...
ucp_proto_rndv_ppln_init(const ucp_proto_init_params_t *init_params)
{
    ...
    *init_params->priv_size = sizeof(*rpriv);
    ...
    if (thresh_elem->proto_config.proto->flags & UCP_PROTO_FLAG_RNDV_ATS) {
        /* Add ATS overhead */
        ...
        status = ucp_proto_rndv_ack_init(init_params, UCP_PROTO_RNDV_ATS_NAME,
                                         &ppln_caps, ppln_overhead,
                                         &rpriv->ack);
    } else {
        /* Not sending ACK - copy ranges as-is */
        rpriv->ack.lane = UCP_NULL_LANE;
        ucp_proto_select_caps_copy(init_params->caps, &ppln_caps);
        status = UCS_OK;
    }
