                      ...
                      ucp_tag_recv_info_t *info)
{
    ...
    if (flags & UCP_RECV_DESC_FLAG_FIRST) {
        ...
        ucs_debug("flags=0x%x", flags);

        if (ucs_unlikely(flags & UCP_RECV_DESC_FLAG_SYNC)) {
            txn_hdr = (flags & UCP_RECV_DESC_FLAG_LAST) ?
                            &((ucp_eager_sync_hdr_t*)data)->txn :
                            &((ucp_eager_sync_first_hdr_t*)data)->txn;
            ucp_tag_eager_sync_send_ack(worker, txn_hdr->sender_uuid,
                                        txn_hdr->tid, 1);
        }
    }
