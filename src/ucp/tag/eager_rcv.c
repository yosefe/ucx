                           ...
                           char *buffer, size_t max)
{
    ...
    case UCP_AM_ID_EAGER_SYNC_ONLY:
        snprintf(buffer, max, "EGRS tag %"PRIx64" tid %d",
                 eagers_hdr->super.super.tag, eagers_hdr->tid);
        header_len = sizeof(*eagers_hdr);
        break;
    case UCP_AM_ID_EAGER_SYNC_FIRST:
        snprintf(buffer, max, "EGRS_F tag %"PRIx64" tid %d len %zu",
                 eagers_first_hdr->super.super.super.tag,
                 eagers_first_hdr->super.tid, eagers_first_hdr->total_len);
