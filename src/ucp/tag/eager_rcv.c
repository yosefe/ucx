                                                ...
                                                size_t length, void *desc)
{
    ...
    if (status == UCS_OK) {
        eagers_hdr = data;
        ucp_tag_eager_sync_send_ack(arg, eagers_hdr->txn.sender_uuid,
                                    eagers_hdr->txn.tid, 0);
    }
    return status;
}
                                                 ...
                                                 size_t length, void *desc)
{
    ...
    if (status == UCS_OK) {
        ...
        ucp_tag_eager_sync_send_ack(arg, eagers_first_hdr->txn.sender_uuid,
                                    eagers_first_hdr->txn.tid, 0);
    }
    return status;
}
                                               ...
                                               size_t length, void *desc)
{
    ...
    if (txn != NULL) {
        ...
        ucs_warn("coould not find transaction %d", ack_hdr->tid);
    }
    return UCS_OK;
}
                           ...
                           char *buffer, size_t max)
{
    ...
    case UCP_AM_ID_EAGER_SYNC_ONLY:
        snprintf(buffer, max, "EGRS tag %"PRIx64" uuid %"PRIx64" tid %d",
                 eagers_hdr->super.super.tag, eagers_hdr->txn.sender_uuid,
                 eagers_hdr->txn.tid);
        header_len = sizeof(*eagers_hdr);
        break;
    case UCP_AM_ID_EAGER_SYNC_FIRST:
        snprintf(buffer, max, "EGRS_F tag %"PRIx64" len %zu uuid %"PRIx64" tid %d",
                 eagers_first_hdr->super.super.super.tag,
                 eagers_first_hdr->super.total_len,
                 eagers_first_hdr->txn.sender_uuid,
                 eagers_first_hdr->txn.tid);
        header_len = sizeof(*eagers_first_hdr);
        break;
    case UCP_AM_ID_EAGER_SYNC_ACK:
        snprintf(buffer, max, "EGRS_A tid %d status '%s'", ack_hdr->tid,
