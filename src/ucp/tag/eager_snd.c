...
static size_t ucp_tag_pack_eager_sync_only_contig(void *dest, void *arg)
{
    ...
    hdr->txn.sender_uuid = req->send.ep->worker->uuid;
    hdr->txn.tid         = req->send.tag.txn.tid;
...
static ucs_status_t ucp_tag_eager_contig_short(uct_pending_req_t *self)
{
    ...
    status = ucp_tag_send_eager_short(ep, req->send.tag.tag, req->send.buffer,
                                      req->send.length);
    if (status != UCS_OK) {
        return status;
    }
...
static ucs_status_t ucp_tag_eager_sync_contig_zcopy_multi(uct_pending_req_t *self)
{
    ...
    first_hdr.txn.sender_uuid       = req->send.ep->worker->uuid;
    first_hdr.txn.tid               = req->send.tag.txn.tid;
