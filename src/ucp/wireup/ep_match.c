...
static inline void ucs_olist_add_tail(ucs_list_link_t *head, ucs_list_link_t *elem)
{
                                       ...
                                       uint64_t dest_uuid, const char *title)
{
    ...
    ucs_olist_add_tail(list, &ucp_ep_ext_gen(ep)->ep_match.list);
                             ...
                             ucp_ep_flags_t exp_ep_flags, const char *title)
{
    ...
    ucs_olist_for_each(ep_ext, list, ep_match.list) {
        ep = ucp_ep_from_ext_gen(ep_ext);
        if (ep->conn_sn == conn_sn) {
            ucs_olist_del(list, &ep_ext->ep_match.list);
...
void ucp_ep_match_remove_ep(ucp_ep_match_ctx_t *match_ctx, ucp_ep_h ep)
{
    ...
    if (ep->flags & UCP_EP_FLAG_DEST_EP) {
        ...
        ucs_olist_del(&entry->unexp_ep_q, &ep_ext->ep_match.list);
    } else {
        ucs_trace("match_ctx %p: remove expected ep %p", match_ctx, ep);
        ucs_olist_del(&entry->exp_ep_q, &ep_ext->ep_match.list);
    }
    ep->flags &= ~UCP_EP_FLAG_ON_HASH;
}
