...
UCS_CLASS_DECLARE(ucp_stub_ep_t, ucp_ep_h, ucp_ep_op_t);
                                    ...
                                    uct_pack_callback_t pack_cb, void *arg)
{
    ...
    wireup_msg_ep = ucs_test_all_flags(stub_ep->state, UCP_EP_STATE_NEXT_EP_LOCAL_CONNECTED|
                         ...
                         stub_ep->next_ep :
                         stub_ep->aux_ep;
    return uct_ep_am_bcopy(wireup_msg_ep, id, pack_cb, arg);
}
                                      ...
                                      const ucp_address_entry_t *address_list)
{
    ...
    status = ucp_wireup_msg_send(ep, UCP_WIREUP_MSG_REQUEST, aux_rsc_index);
    if (status != UCS_OK) {
