 ...
 */
static ucs_status_t ucp_wireup_select_transport(ucp_ep_h ep,
                                                const ucp_address_entry_t *address_list,
                                                unsigned address_count,
                                                ucp_wireup_score_function_t score_func,
                                                uint64_t remote_pd_flags,
                                                int show_error,
                                                const char *title,
...
static void ucp_wireup_add_lane_desc(ucp_wireup_lane_desc_t *lane_descs,
                                     ...
                                     double score, uint32_t usage)
{
    ...
    lane_descs[*num_lanes_p].score        = score;
...
static int ucp_wireup_compare_lane_desc(const void *elem1, const void *elem2)
{
    const ucp_wireup_lane_desc_t *desc1 = elem1;
    const ucp_wireup_lane_desc_t *desc2 = elem2;
    return (int)desc1->dst_pd_index - (int)desc2->dst_pd_index;
}
                               ...
                               uint32_t usage)
{
    ...
    /* Remove all occurrences of the remote pd from the address list, to avoid
     * selecting the same remote pd again.
     */
    ucp_wireup_address_list_remove_pd(address_list_copy, address_count, dst_pd_index);
                                                 ...
                                                 unsigned *addr_indices)
{
    ...
    /* Sort lanes according to pd index */
    qsort(lane_descs, num_lanes, sizeof(*lane_descs), ucp_wireup_compare_lane_desc);

    UCS_STATIC_ASSERT(UCP_MAX_PDS <= sizeof(ep->dest_rma_pds) * 8);
    UCS_STATIC_ASSERT(UCP_MAX_PDS <= sizeof(ep->dest_amo_pds) * 8);
    ...
    for (lane = 0; lane < num_lanes; ++lane) {
        ...
        if (lane_descs[lane].usage & UCP_WIREUP_LANE_USAGE_AM) {
                                           ...
                                           sizeof(reason)))
            {
                ...
                ucs_debug("not using lane[%d] "UCT_TL_RESOURCE_DESC_FMT
        ...
        if (lane_descs[lane].usage & UCP_WIREUP_LANE_USAGE_RMA) {
            ucs_assert(!(ep->dest_rma_pds & UCS_BIT(dst_pd_index)));
            ep->dest_rma_pds  |= UCS_BIT(dst_pd_index);
            key.rma_lanes_map |= UCS_BIT(lane);
        }
        if (lane_descs[lane].usage & UCP_WIREUP_LANE_USAGE_AMO) {
            ucs_assert(!(ep->dest_amo_pds & UCS_BIT(dst_pd_index)));
