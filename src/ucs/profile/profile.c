...
static void ucs_profile_write()
{
    ...
    /* write locations */
    status = ucs_profile_file_write_data(fd, ucs_profile_global_ctx.locations,
                             ...
                             volatile int *loc_id_p)
{
    ...
    for (index = 0; index < ucs_profile_global_ctx.num_locations; ++index) {
        loc = &ucs_profile_global_ctx.locations[index];

        if ((type == loc->type) && (line == loc->line) &&
            !strcmp(loc->name, name) && !strcmp(loc->file, basename(file)) &&
            !strcmp(loc->function, function)) {
            *loc_id_p = loc_id = index + 1;
            goto out_unlock;
        }
    }

    loc_index = ucs_profile_global_ctx.num_locations++;

    /* Reallocate array if needed */
    if (ucs_profile_global_ctx.num_locations > ucs_profile_global_ctx.max_locations) {
        ucs_profile_global_ctx.max_locations = 2 * ucs_profile_global_ctx.num_locations;
    ...
    loc             = &ucs_profile_global_ctx.locations[loc_index];
