                            ...
                            void **address_p)
{
    ...
    status = uct_mm_iface_mapper_call(iface, mem_attach, seg_id,
                         ...
                         void **address_p)
{
    ...
    return uct_mm_ep_attach_remote_seg(ep, seg_id, address_p);
}
...
static UCS_CLASS_INIT_FUNC(uct_mm_ep_t, const uct_ep_params_t *params)
{
    ...
    status = uct_mm_ep_get_remote_seg(self, addr->fifo_seg_id, &fifo_ptr);
    if (fifo_ptr == NULL) {
    ...
    UCT_MM_IFACE_ASSERT_ALIGNED(self->fifo_ctl);
...
retry:
    ...
    case UCT_MM_SEND_AM_BCOPY:
        ...
        status = uct_mm_ep_get_remote_seg(ep, elem->desc.seg_id, &base_address);
        if (ucs_unlikely(status != UCS_OK)) {
            return status;
        }

        length       = pack_cb(base_address + elem->desc.offset, arg);
        elem->flags &= ~UCT_MM_FIFO_ELEM_FLAG_INLINE;
        elem->length = length;

        uct_iface_trace_am(&iface->super.super, UCT_AM_TRACE_TYPE_SEND, am_id,
                           base_address + elem->desc.offset, length,
                           "TX: AM_BCOPY");
