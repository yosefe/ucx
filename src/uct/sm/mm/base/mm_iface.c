                                             ...
                                             uct_iface_addr_t *addr)
{
    ...
    iface_addr->fifo_seg_id = iface->recv_fifo_mem.seg_id;
    uct_mm_md_mapper_ops(md)->iface_addr_pack(md, iface_addr + 1);
                                        ...
                                        uct_mem_h memh)
{
    ...
    desc->info.offset = UCS_PTR_BYTE_OFFSET(desc + 1, iface->rx_headroom) -
                        seg->address;
}
...
static void uct_mm_iface_free_rx_descs(uct_mm_iface_t *iface, unsigned num_elems)
{
    ...
    for (i = 0; i < num_elems; i++) {
        ...
        desc = (uct_mm_recv_desc_t*)(elem->desc_data - iface->rx_headroom) - 1;
        ucs_mpool_put(desc);
    }
}
                                ...
                                void **fifo_elems_p)
{
    ...
    *fifo_ctl_p   = fifo_ctl;
    *fifo_elems_p = (void*)fifo_ctl + UCT_MM_FIFO_CTL_SIZE;
}

static ucs_status_t uct_mm_iface_fifo_alloc(uct_mm_iface_t *iface,
                                            uct_mm_iface_config_t *config)
{
    ucs_status_t status;

    /* allocate the receive FIFO */
    iface->recv_fifo_mem.address = NULL;
    iface->recv_fifo_mem.length  = UCT_MM_GET_FIFO_SIZE(iface);
    status = uct_mm_iface_mapper_call(iface, mem_alloc, &iface->recv_fifo_mem,
                                      0, "mm_recv_fifo");
    if (status != UCS_OK) {
        return status;
    }

    /* mapper must return aligned memory */
    UCT_MM_IFACE_ASSERT_ALIGNED(iface->recv_fifo_mem.address);
...
static void uct_mm_iface_fifo_release(uct_mm_iface_t *iface)
{
    uct_mm_iface_mapper_call(iface, mem_free, &iface->recv_fifo_mem);
}
                           ...
                           const uct_iface_config_t *tl_config)
{
    ...
    status = uct_mm_iface_fifo_alloc(self, mm_config);
    if (status != UCS_OK) {
        goto err;
    }
...
err_free_fifo:
    uct_mm_iface_fifo_release(self);
err:
    return status;
}
