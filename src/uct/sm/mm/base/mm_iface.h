...
#define UCT_MM_IFACE_ASSERT_ALIGNED(_ptr) \
    ucs_assert_always((((uintptr_t)(_ptr)) % UCS_SYS_CACHE_LINE_SIZE) == 0);
...
typedef struct uct_mm_desc_info {
    ...
    size_t                  offset;           /* offset inside the shared memory
                                                 segment */
...
typedef struct uct_mm_fifo_element {
    ...
    void                      *desc_data;     // TODO separate array
