...
static ucs_status_t uct_mm_iface_wakeup_get_fd(uct_wakeup_h tl_wakeup, int *fd_p)
{
    uct_mm_iface_t *iface = ucs_derived_of(tl_wakeup, uct_mm_wakeup_t)->iface;
    *fd_p = iface->signal_fd;
    return UCS_OK;
}

static ucs_status_t uct_mm_iface_wakeup_drain(uct_wakeup_h tl_wakeup)
{
    uct_mm_iface_t *iface = ucs_derived_of(tl_wakeup, uct_mm_wakeup_t)->iface;
    uint32_t dummy;
    int ret;
    ...
    if (errno != EAGAIN) {
        ucs_error("failed to retreive message from signal pipe: %m");
...
static UCS_CLASS_DEFINE_NEW_FUNC(uct_mm_wakeup_t, uct_wakeup_t, uct_iface_h, unsigned);
static UCS_CLASS_DEFINE_DELETE_FUNC(uct_mm_wakeup_t, uct_wakeup_t);

static uct_wakeup_ops_t uct_mm_wakeup_ops = {
    .get_fd = uct_mm_iface_wakeup_get_fd,
    .arm    = (void*)ucs_empty_function_return_success,
    .drain  = uct_mm_iface_wakeup_drain,
    .wait   = NULL,
    .signal = NULL,
    .close  = UCS_CLASS_DELETE_FUNC_NAME(uct_mm_wakeup_t),
};

UCS_CLASS_INIT_FUNC(uct_mm_wakeup_t, uct_iface_h tl_iface, unsigned events)
{
    uct_mm_iface_t *iface = ucs_derived_of(tl_iface, uct_mm_iface_t);

    UCS_CLASS_CALL_SUPER_INIT(uct_wakeup_t, &uct_mm_wakeup_ops)
    self->iface     = iface;
    return UCS_OK;
}

UCS_CLASS_CLEANUP_FUNC(uct_mm_wakeup_t)
{
}

UCS_CLASS_DEFINE(uct_mm_wakeup_t, uct_wakeup_t);
...
static uct_iface_ops_t uct_mm_iface_ops = {
    ...
    .iface_wakeup_open   = UCS_CLASS_NEW_FUNC_NAME(uct_mm_wakeup_t),
...
static void uct_mm_iface_singal_handler(void *arg)
{
    ...
    if (ucs_atomic_cswap32(&iface->on_progress, 0, 1) == 0) {
                           ...
                           const uct_iface_config_t *tl_config)
{
    ...
    ucs_async_set_event_handler(UCS_ASYNC_MODE_THREAD, self->signal_fd,
                                POLLIN, uct_mm_iface_singal_handler, self,
                                worker->async);
...
static UCS_CLASS_CLEANUP_FUNC(uct_mm_iface_t)
{
    ...
    if (ucs_atomic_cswap32(&self->on_progress, 1, 0) == 1) {
