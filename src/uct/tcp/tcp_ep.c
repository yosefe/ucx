                            ...
                            uct_pack_callback_t pack_cb, void *arg)
{
    ...
    UCT_CHECK_LENGTH(hdr->length, iface->config.max_bcopy, "am_bcopy");

    UCS_DEBUG_DATA(hdr->sn = ep->sn);

    status = uct_tcp_send(ep->fd, hdr, sizeof(*hdr) + hdr->length);
    if (status < 0) {
        ret = status;
        goto out_put;
    }

    UCS_DEBUG_DATA(++ep->sn);
    uct_iface_trace_am(&iface->super, UCT_AM_TRACE_TYPE_SEND, hdr->am_id,
                        hdr + 1, hdr->length, "SEND fd %d" UCS_DEBUG_DATA(" sn %u"),
                        ep->fd, UCS_DEBUG_DATA(hdr->sn));
