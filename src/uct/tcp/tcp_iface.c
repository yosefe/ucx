...
static ucs_config_field_t uct_tcp_iface_config_table[] = {
  ...
  {"MAX_EVENTS", "8",
   "Maximal events to poll for in worker progress. 0 - no polling.",
   ucs_offsetof(uct_tcp_iface_config_t, max_events), UCS_CONFIG_TYPE_UINT},
...
void uct_tcp_iface_release_am_desc_safe(uct_tcp_iface_t *iface,
...
static ucs_status_t uct_tcp_iface_event_fd_get(uct_iface_t *tl_iface, int *fd_p)
{
    uct_tcp_iface_t *iface = ucs_derived_of(tl_iface, uct_tcp_iface_t);
                           ...
                           const uct_iface_config_t *tl_config)
{
    ...
    ucs_queue_head_init(&self->recv_am_queue);
