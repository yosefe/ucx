...
static ucs_status_t uct_tcp_do_io(int fd, void *data, size_t length,
                                  uct_tcp_io_func_t io_func, const char *name,
                                  ucs_status_t status_no_data)
{
    ssize_t ret;

    for (;;) {
        ret = io_func(fd, data, length, 0);
        if (ret == length) {
            return UCS_OK;
        } else if (ret == 0) {
...
{
    return uct_tcp_do_io(fd, (void*)data, length, (uct_tcp_io_func_t)send,
                         "send", UCS_ERR_NO_RESOURCE);
}

ucs_status_t uct_tcp_recv(int fd, void *data, size_t length)
{
    return uct_tcp_do_io(fd, data, length, recv, "recv", UCS_ERR_NO_PROGRESS);
