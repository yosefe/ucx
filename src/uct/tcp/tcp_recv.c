...
static unsigned uct_tcp_iface_poll_rx(uct_tcp_iface_t *iface, int fd, int sync);
                                    ...
                                    uct_tcp_am_desc_t *desc)
{
    ...
    if (status == UCS_OK) {
        uct_tcp_iface_release_am_desc_safe(iface, desc);
...
unsigned uct_tcp_iface_progress(uct_iface_h tl_iface)
{
    ...
    /* First, dispatch queued active messages */
    ucs_queue_for_each_extract(desc, &iface->recv_am_queue, queue,
                               count < iface->config.max_poll) {
    ...
    while ((idx < nevents) && (count < iface->config.max_poll)) {
        poll_count = uct_tcp_iface_poll_rx(iface, events[idx].data.fd, 1);
        count += poll_count;
        if (poll_count == 0) {
...
{
    uct_tcp_iface_poll_rx((uct_tcp_iface_t*)arg, fd, 0);
}
...
ucs_status_t uct_tcp_iface_connection_accepted(uct_tcp_iface_t *iface, int fd)
{
    ...
    status = ucs_async_set_event_handler(worker->async->mode, fd, POLLIN|POLLERR,
...
static unsigned uct_tcp_iface_poll_rx(uct_tcp_iface_t *iface, int fd, int sync)
{
    ...
    if ((iface->super.am[hdr->am_id].flags & UCT_AM_CB_FLAG_SYNC) && !sync) {
        ucs_queue_push(&iface->recv_am_queue, &desc->queue);
    } else {
        uct_tcp_iface_invoke_am(iface, desc);
    }
...
void uct_tcp_iface_recv_cleanup(uct_tcp_iface_t *iface)
{
    ...
    /* Remove deferred active messages */
    ucs_queue_for_each_extract(desc, &iface->recv_am_queue, queue, 1) {
        ucs_mpool_put(desc);
    }
}
