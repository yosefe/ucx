...
protected:
    ...
    public:
        bistro_patch(const char* symbol, void *hook)
        {
            ...
            status = ucm_bistro_patch(func_ptr, hook, &m_rp);
            ASSERT_UCS_OK(status);
...
UCS_TEST_SKIP_COND_F(malloc_hook, bistro_patch, RUNNING_ON_VALGRIND) {
    ...
    void *func_ptr = ucm_reloc_get_orig(symbol, (void*)bistro_hook<0>::munmap);
    ucs_status_t status;
    munmap_f_t *munmap_f;
    void *ptr;
    int res;
    ...
    status = ucm_bistro_patch(func_ptr, (void*)bistro_hook<0>::munmap, &rp);
    ASSERT_UCS_OK(status);
