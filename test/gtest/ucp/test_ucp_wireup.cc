...
private:
    ...
    ucp_mem_h  m_memh_receiver, m_memh_sender;
...
void test_ucp_wireup::init() {
    ...
    if (GetParam().variant == TEST_RMA) {
        ...
        status = ucp_mem_map(receiver().ucph(), &params, &m_memh_receiver);
        ASSERT_UCS_OK(status);

        status = ucp_mem_map(sender().ucph(), &params, &m_memh_sender);
        ASSERT_UCS_OK(status);
    }
}
...
void test_ucp_wireup::cleanup() {
    if (GetParam().variant == TEST_RMA) {
        m_rkeys.clear();
        ucp_mem_unmap(sender().ucph(),   m_memh_sender);
        ucp_mem_unmap(receiver().ucph(), m_memh_receiver);
    }
    ucp_test::cleanup();
}
                             ...
                             int repeat)
{
    if ((GetParam().variant == TEST_TAG) || (GetParam().variant == TEST_STREAM))
    {
        ...
        for (size_t i = 0; i < length; ++i) {
            while (m_recv_data[i] != SEND_DATA + repeat - 1) {
                progress();
            }
        }
    }
                                ...
                                ucp_ep_h recv_ep, size_t length, int repeat)
{
    ...
    send_nb(send_ep, length, repeat, send_reqs);
    recv_b (recv_worker, recv_ep, length, repeat);
    waitall(send_reqs);
    m_rkeys.clear();
}
...
UCS_TEST_P(test_ucp_wireup_2sided, multi_ep_2sided) {
    ...
    for (unsigned i = 0; i < count; ++i) {
        sender().connect(&receiver(), get_ep_params(), i);
        if (!is_loopback()) {
            receiver().connect(&sender(), get_ep_params(), i);
        }
    }

    for (unsigned i = 0; i < count; ++i) {
        send_recv(sender().ep(0, i), receiver().worker(), receiver().ep(0, i), 8, 1);
        send_recv(receiver().ep(0, i), sender().worker(), sender().ep(0, i), 8, 1);
    }
}
